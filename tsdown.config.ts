/**
 * tsdown build for dsh-compact-button: the host-half lib/lib index.js
 * (ESM node, a no-op apply) plus the two browser client bundles
 ...
 *   the package-name id `dsh-compact-button` (the client-modules compose
 ...
 *   `dsh-external/dsh-compact-button` (the registry browser-side `arrive()`
...
function clientBundle(pluginId: string, entryFile: string): UserConfig {
  return {
    ...
    // Node entry even though this bundle runs in the browser.
    inputOptions: {
      resolve: {
...
export default [
  {
    ...
    dts: false,
    // clean stays off: the build script removes lib/ wholesale before tsdown
    // (and `watch` must never wipe the other channel's artifacts).
    clean: false,
  },
  // Official profile channel: bundle id = package name (package.json `name`).
  clientBundle('dsh-compact-button', 'client.js'),
  // Plugin-registry channel: bundle id = manifest id (dsh.plugin.json `id`).
  clientBundle('dsh-external/dsh-compact-button', 'client-registry.js'),
